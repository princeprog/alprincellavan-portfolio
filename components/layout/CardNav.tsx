...
const CardNav: React.FC<CardNavProps> = ({
  ...
  const finalBaseColor = baseColor ?? currentTheme.base;
  const finalMenuColor = menuColor ?? currentTheme.menu;
  const finalButtonBgColor = buttonBgColor ?? currentTheme.buttonBg;
  const finalButtonTextColor = buttonTextColor ?? currentTheme.buttonText;
  ...
  return (
    <div
      className={`card-nav-container absolute left-1/2 -translate-x-1/2 w-[50%] max-w-[800px] z-[99] top-[1.2em] md:top-[2em] ${className}`}
    >
      <nav
        ref={navRef}
        className={`card-nav ${isExpanded ? 'open' : ''} block h-[20px] p-0 rounded-xl shadow-md relative overflow-hidden will-change-[height]`}
              ...
              <div className="nav-card-links mt-auto flex flex-col gap-[2px]">
                {item.links?.map((lnk, i) => (
                  <a
                    ...
                    href={lnk.href}
                    onClick={(e) => handleNavigation(lnk.href, e)}
